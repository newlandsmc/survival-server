...
do
  ...
  aws --profile default s3 cp --no-progress $backupDir/$sqlDatabase-$currentDate.sql.gz s3://asthonia-prod-backups-sql/
...
do
  ...
  aws --profile default s3 cp --no-progress $backupDir/server-$world-$currentDate.tar.gz s3://asthonia-prod-backups-server/
...
aws --profile default s3 cp --no-progress $backupDir/server-plugins-$currentDate.tar.gz s3://asthonia-prod-backups-server/
